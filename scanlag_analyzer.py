...
def inittiate_analysis(analysis_dir,scanned_images):
    ...
    return os.path.join(analysis_dir,s)
...
#Plate indentification and cropping
           
def find_plates(parent_dir,scanned_image_folder):
    ...
    for (i,c) in enumerate(cnts):
            ...
            cv2.circle(image_read,(int(cX),int(cY)),int(radius),
                       (0,255,0),3)
...
def crop_plates(folder,out_folder, circles,names):
    ...
    times = [int(t.split(".")[0].split("_")[1]) for t in os.listdir(folder) if t.split(".")[-1]=="png"]
    image_paths = [os.path.join(folder,t) for t in os.listdir(folder) if t.split(".")[-1]=="png"]
    t0 = min(times)

    names_pos = {}
    for plates,pos in circles.items():
        if names[plates]:
            names_pos[names[plates]]=pos
        else:
            pass
...
def plot_time_vs_growth(data,y, cell_types):
    ax = plt.subplot()
    groups = data.groupby("Cell_type")
    for gr,group in groups:
        if gr in cell_types:
            color = colors[cell_types.index(gr)]
            sns.regplot(x="Time",y = y,data = group,x_estimator=np.mean,truncate = False,fit_reg = False, label = gr,ax=ax)#, order = 3)
    plt.legend()
    plt.show()
...
"""
#circle = np.array([[1098,1374,550],[1164,3138,550],[3360,3144,550],[1146,4848,550],[3234,4854,550]])
name = {"1":None,"2":None,"3":None,"4":None,"5":None,"6":None}
time = [1424,1057,1424,735,873]
"""
